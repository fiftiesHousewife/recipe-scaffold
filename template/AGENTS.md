...
- **`./gradlew addRecipe`, `verifyGates`, `upgradeSkills`, `upgradeBuildLogic`, `doctor`** are thin Gradle wrappers around the upstream `recipe-scaffold` subcommands. They shell out to `jbang` on PATH and forward args via `--args="…"`. `init` is not wrapped (no project = no Gradle context). Edit `recipe-library.gradle.kts` if you need to pin the upstream catalog reference to a specific tag instead of `main`.
