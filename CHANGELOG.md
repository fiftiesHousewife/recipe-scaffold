...
- **`rewrite-groovy` parser** in the scaffolded project's `recipe-library` runtime classpath, declared explicitly in `libs.versions.toml`. Lets recipes target standalone `.groovy` files; previously available only transitively via `rewrite-gradle` for Groovy DSL build scripts.
- **Inner README** (`template/README.md`) gained a "What this scaffold parses" table covering all bundled parsers (Java/Groovy/Gradle/TOML/Properties/YAML/Refaster), a Groovy DSL row in "Supported project shapes", a version-catalog mention in Development, and a References table pointing to OpenRewrite, Gradle convention plugins, version catalogs, and the upstream scaffolder.
