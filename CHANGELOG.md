...
- **Inner README** (`template/README.md`) gained a "What this scaffold parses" table covering all bundled parsers (Java/Gradle/TOML/Properties/YAML/Refaster), a Groovy DSL build-script row in "Supported project shapes", a version-catalog mention in Development, and a References table pointing to OpenRewrite, Gradle convention plugins, version catalogs, and the upstream scaffolder. Groovy DSL build scripts are supported via `rewrite-gradle`'s transitive `rewrite-groovy` pull; standalone `.groovy` files are out of scope.
- **`central-roundtrip` job** in `template/.github/workflows/release.yml`. Runs after `publish` succeeds: scaffolds a throwaway consumer that depends on the just-published GAV from `mavenCentral()` (not `mavenLocal`) and runs `rewriteResolveDependencies`. Retries with backoff to absorb Central propagation lag. `template/SMOKE_TEST.md` reframed as the manual fallback for releases cut outside CI.
